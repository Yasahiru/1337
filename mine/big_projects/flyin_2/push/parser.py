...
class Parser:
    z_pattern = r"^(start_hub|end_hub|hub):\s+(.+?)\s+(-?\d+)"
    z_pattern += r"\s+(-?\d+)(?:\s+\[(.*)\])?$"
    ...
    def _read_file(self, filename: str) -> List[tuple[int, str]]:
    ...
    def parse_zone(self, line_number: int, line: str) -> None:
    ...
    def is_coords_exist(self, coords: Tuple[int, int]) -> bool:
